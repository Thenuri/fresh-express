   ...
   <div class="bg-white shadow rounded-lg p-4 sm:p-6 xl:p-8">
         ...
         <table class="items-center w-full bg-transparent border-collapse">
            ...
            <tbody class="divide-y divide-gray-100">
               ...
               <tr class="text-gray-500">
                  <td class="border-t-0 px-4 align-middle text-sm font-normal whitespace-nowrap p-4 text-left">{{ $product->id }}</td>
                  <td class="border-t-0 px-4 align-middle text-xs font-medium text-gray-900 whitespace-nowrap p-4">{{ $product->name }}</td>
                  <td class="border-t-0 px-4 align-middle text-xs font-medium text-gray-900 whitespace-nowrap p-4">{{ $product->quantity }}</td>
                  <td class="border-t-0 px-4 align-middle text-xs font-medium text-gray-900 whitespace-nowrap p-4">{{ $product->price }}</td>
                  <td class="border-t-0 px-4 align-middle text-xs font-medium text-gray-900 whitespace-nowrap p-4 flex">
                     ...
                     <div class="mt-5">
                        <x-danger-button wire:click="confirmProductUpdate({{$product->id}})" wire:loading.attr="disabled" class=" bg-amber-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-amber-500 active:bg-amber-700  focus:ring-amber-500 ">
                            {{ __('Update') }}
                        </x-danger-button>
                     </div>
                  </td>
      ...
      <x-confirmation-modal wire:model="confirmProductDeletion">
         ...
         <x-slot name="footer">
             ...
             <x-danger-button class="ml-3" wire:click="deleteproduct({{$confirmProductDeletion}})" wire:loading.attr="disabled">
                 {{ __('Delete') }}
             </x-danger-button>
         </x-slot>
     ...
     <x-dialog-modal wire:model="confirmProductAdd">
      ...
      <x-slot name="content">
         ...
         <div class="col-span-6 sm:col-span-4 mt-4">
            ...
            <x-input id="name" type="text" class="mt-1 block w-full" wire:model.defer="product.quantity" />
            <x-input-error for="product.quantity" class="mt-2" />
         </div>
         <div class="col-span-6 sm:col-span-4 mt-4">
            <x-label for="name" value="{{ __('Price') }}" />
            <x-input id="name" type="text" class="mt-1 block w-full" wire:model.defer="product.price" />
